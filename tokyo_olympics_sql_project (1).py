...
df_EnteriesGender.isnull().sum()                           #### EntriesGender dataframe
...
df_Coaches.drop(columns=["Event"], inplace=True)                             ### Coaches dataframe

import sqlite3

conn = sqlite3.connect("/content/tokyo_olympics.db")
cursor = conn.cursor()
...
db_path = "tokyo_olympics.db"  # adjust if needed
...
df_athletes.to_sql("athletes", connect, if_exists="replace", index=False) ### athletes table formation in database
df_Coaches.to_sql("Coaches", connect, if_exists="replace", index=False)  ### Coaches table formation in database
df_EntriesGender.to_sql("EntriesGender", connect, if_exists="replace", index=False) ### EnteriesGender table formation in database
df_Medals.to_sql("Medals", connect, if_exists="replace", index=False) ### Medals table formation in database
df_Teams.to_sql("Teams", connect, if_exists="replace", index=False) ### Teams table formation in database
...
pd.read_sql_query("""select * from EnteriesGender limit 3;""", connect)   ### EnteriesGender table information
...
FROM Athletes a
JOIN EnteriesGender e ON a.Discipline = e.Discipline
GROUP BY a.NOC
...
# which countries are participated in olympics but didnot wind any medals?
